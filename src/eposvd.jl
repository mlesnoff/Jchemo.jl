...
"""
    eposvd(D; nlv = 1)
Compute an orthogonalization matrix for calibration transfer 
    of spectral data.
* `D` : Data (m, p) containing the detrimental information 
    on which spectra (rows of a matrix X) have to be orthogonalized.
Keyword arguments:
* `nlv` : Nb. of first loadings vectors of `D` considered for the 
    orthogonalization.

The objective is to remove some detrimental information 
(e.g. humidity patterns in signals, multiple spectrometers, etc.) 
from a X-dataset (n, p).  The detrimental information is defined 
...
Matrix `D` can be built from many methods. For instance, 
    two common methods are:
* EPO (Roger et al. 2003, 2018): `D` is built from a set of 
    differences between spectra collected under different 
    conditions. 
* TOP (Andrew & Fearn 2004): Each row of `D` is the mean spectrum 
    computed for a given spectrometer instrument.
