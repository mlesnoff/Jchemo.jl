"""
    kplsqda(; kwargs...)
    kplsqda(X, y; kwargs...)
    kplsqda(X::Matrix{Q}, y::Vector{String}, weights::ProbabilityWeights{Q}; kwargs...) where Q <: Float
KPLS-QDA.
* `X` : X-data (n, p).
...
Same as function `plsqda` (PLS-QDA) except that a kernel PLSR (function `kplsr`), instead of a PLSR (function `plskern`), 
...
function kplsqda(X::Matrix{Q}, y::Vector{String}, weights::ProbabilityWeights{Q}; kwargs...) where Q <: Float
    par = recovkw(ParKplsqda{Q}, kwargs).par
