...
function default(algo::Function)
    ...
    in((dtairpls,)).(algo) ? dump(Jchemo.ParDtairpls()) : nothing
    in((dtarpls,)).(algo) ? dump(Jchemo.ParDtarpls()) : nothing
    ...
    in((snipals, snipalsh, snipalsmix)).(algo) ? dump(Jchemo.ParSnipals()) : nothing
