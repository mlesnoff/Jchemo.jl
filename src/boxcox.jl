...
```
""" 
function boxcox(x::Vector{Q}; lims::Vector{Q} = [-3.; 3], npoint::Int = 1000) where Q <: Float
    if any(x .<= 0)
...
"""
    boxcox_transf(x::Vector{Q}, lb::Q) where Q <: Float
    boxcox_transf!(x::Vector{Q}, lb::Q) where Q <: Float
...
Journal of the Royal Statistical Society, Series B 26(2), 211--252.
""" 
function boxcox_transf(x::Vector{Q}, lb::Q) where Q <: Float 
    vx = copy(x)
    boxcox_transf!(vx, lb)
    vx
end
