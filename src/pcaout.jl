...
The objective is to remove the effect of multivariate `X`-outliers that have potentially bad leverages. 
Observations (`X`-rows) receive weights depending on two outlyingness indicators:
