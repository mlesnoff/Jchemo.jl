...
For `nsim = 0`, this is the Croux & Ruiz-Gazen (C-R, 2005) PCA algorithm that uses a projection pursuit (PP) method. 
Data `X` are robustly centered by the spatial median, and the observations are projected to the "PP" directions 
defined by the observations (rows of `X`) after they are normed. The first PCA loading vector is the direction 
(within the PP directions) that maximizes a given 'projection index', here the median absolute deviation (MAD). 
Then, `X` is deflated to this loading vector, and the process is re-run to define the next loading vector. And so on. 

A possible extension of this algorithm is to randomly simulate additionnal candidate PP directions to the n row 
observations. If `nsim > 0`, the function simulates `nsim` additional PP directions to the n initial ones, as 
